...
class PurchaseOrder(models.Model):
    ...
    def get_model_lines(self):
        ...
        model_sequence = 0
        for line in self.order_line.search([['ad_is_model_variant', '=', True], ['order_id', '=', my_id]], order='id'):
            ...
            if model.id not in bar['models'].keys():
                ...
                bar['models'][model.id] = {
                    ...
                    'products': values,
                    'taxes': ', '.join(map(lambda x: x.name, line.taxes_id)),
                    'total_quantity': 0.0,
                    'total_price': 0.0,
                    'image': image,
            ...
            for prod in bar_model['products']:
                if prod['id'] == product.id:
                    prod['quantity'] = line.product_qty
        ...
        for bar in measurebars:
            bar['models'] = sorted(
                bar['models'].values(), key=lambda x: x['model_sequence'])
